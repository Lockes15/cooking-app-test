...
const useCustomTimer = (initialTime, timerId, onTimerStop) => {
  const { selectedAlarmSound } = useContext(SettingsContext);
  const [time, setTime] = useState(() => {
    const savedTime = localStorage.getItem(`custom-timer-${timerId}-time`);
    return savedTime ? parseInt(savedTime) : initialTime;
  });
  const [isRunning, setIsRunning] = useState(() => {
    const savedIsRunning = localStorage.getItem(`custom-timer-${timerId}-isRunning`);
    return savedIsRunning ? JSON.parse(savedIsRunning) : false;
  });
  const [startTime, setStartTime] = useState(() => {
    const savedStartTime = localStorage.getItem(`custom-timer-${timerId}-startTime`);
  ...
  useEffect(() => {
    localStorage.setItem(`custom-timer-${timerId}-time`, time);
    localStorage.setItem(`custom-timer-${timerId}-isRunning`, isRunning);
    localStorage.setItem(`custom-timer-${timerId}-startTime`, startTime);
